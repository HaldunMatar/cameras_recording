...
class _CamerasScreenState extends State<CamerasScreen> {
  ...
  void _toggle(CameraInfo cam) {
    ...
    setState(() {
      _selected.contains(cam.name)
          ? _selected.remove(cam.name)
          : _selected.add(cam.name);
    });
  }
  ...
  Future<void> _startRecording() async {
    ...
    // Show the duration + format picker sheet.
    // Returns a record (durationSeconds, format) or null if cancelled.
  ...
  Widget _buildBody() {
    if (_loading) {
      return ListView(
        ...
        children: List.generate(4, (_) => const _CamSkeleton()),
      );
    }
    if (_error != null) {
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('No cameras configured',
              style: TextStyle(
                fontSize: 16, color: AppColors.text2, fontWeight: FontWeight.w500)),
  ...
  void _showAddCamera() {
    ...
    final urlCtrl   = TextEditingController(
        text: 'rtsp://admin:pass@192.168.x.x:554/');
...
// ─────────────────────────────────────────────────────────────────────────────
// Duration + format picker bottom sheet
// Returns (int durationSeconds, String format) or null if dismissed
// ─────────────────────────────────────────────────────────────────────────────
...
class _DurationPickerSheetState extends State<_DurationPickerSheet> {
  ...
  int get _finalSeconds {
    if (_presets[_selectedIndex].$2 != -1) return _presets[_selectedIndex].$2;
      ...
      child: Column(
        ...
        children: [
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 20),
            child: Text(
              'Each file is this long. Recording auto-restarts until you press Stop.',
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Text(_presets[i].$1,
                      style: TextStyle(
                        ...
                        color: active ? AppColors.red : AppColors.text2)),
                  ),
                );
              }),
            ),
          ...
          Padding(
            ...
            child: Text(
              _format == 'ts'
                  ? '.TS — crash-safe, always readable, best for 24/7'
                  : '.MP4 — standard format, smaller files, best compatibility',
            ...
            child: SizedBox(
              ...
              child: FilledButton.icon(
                ...
                onPressed: _finalSeconds < 1
                    ...
                    : () => Navigator.pop(
                          context,
                          (_finalSeconds, _format),   // ← returns the record
                        ),
                icon: const Icon(Icons.fiber_manual_record, size: 16),
                label: Text(
                  'Start · ${_fmtDur(_finalSeconds)} · .${ _format.toUpperCase()}',
                  style: const TextStyle(
...
class _FmtBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: active ? AppColors.red.withOpacity(0.12) : AppColors.bg3,
        borderRadius: BorderRadius.circular(9),
        border: Border.all(
          color: active
              ? AppColors.red.withOpacity(0.5)
    ...
    child: Column(
      ...
      children: [
        ...
        const Text('Cannot reach server',
          style: TextStyle(
            fontSize: 16, fontWeight: FontWeight.w500, color: AppColors.text2)),
        const SizedBox(height: 6),
        Padding(
